...
enum { SchemeNorm, SchemeSel, SchemeMonSel }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeMonSel : SchemeNorm]);
...
#endif /* __OpenBSD__ */
	scan();
	for (int i = 0; i < LENGTH(autostart_cmds); i++)
		system(autostart_cmds[i]);
	run();
	cleanup();
	XCloseDisplay(dpy);
	return EXIT_SUCCESS;
